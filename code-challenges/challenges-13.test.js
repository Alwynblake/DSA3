...
// Solution code here...
  let longIndex = -1; //longIndex = -1
  let longString = 0; //longString = 0
  arr.forEach (( item, i) => { //pass value of arr forEach item at index of i
    if (item.length > longString) { //if item.length is greater than longString
      longString = item.length; //longString = item.length
      longIndex = i;// longe index is = to i
    }
  });
  return longIndex; // we return longIndex
};
...
const firstLetters = (arr) => {
  // Solution code here...
  return arr.map(value => value[0]); // we return the arr.map it and pass value to a function of value at index 0
};
...
const findHappiness = (arr) => {
  // Solution code here...
  return arr.filter((coffee) => /:\)/.test(coffee));//we return the arr.filter at value of coffee because coffee makes me happy. We then take that and pass a function with the use of regex and do a .test to the value of coffee.
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  const result = []; // results is an empty array to pass value
  arr.forEach ( roledexNum => { // we take the arr.forEach and pass it a value
    const chopNumber = roledexNum.replace(/\D+/g, ''); // now we remove what we don't want from the array
    result.push(chopNumber);
  });
  return result;
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  const results = []; // empty array to pass results
