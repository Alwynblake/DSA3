...
const sortByChildren = (charArray) => { // charArray is equal to sortByChildren
  ...
  charArray.sort((a, b) => { // we want to sort A and B
    if (a.children.length < b.children.length) { // if a.children.length is less than b.children.length we return -1.
      return -1;
    } else if (a.children.length > b.children.length) { // else if a.children.length is greater than b.children.length return 1
      return 1;
    } else if (a.children.length === b.children.length) { // now that we sort the children we do a strict equals too comparison
      if (a.house < b.house) { // now that we sort the children we check to see if the houses name is alphabetical
        return -1;
