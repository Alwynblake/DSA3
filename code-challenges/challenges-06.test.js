...
const sortByChildren = (charArray) => {
  // Solution code here...
  charArray.sort(function(leftPerson, rightPerson){
    leftPerson = leftPerson.children.length;
    rightPerson = rightPerson.children.length;
    if (leftPerson > rightPerson){
      ...
      if (leftPerson.name > rightPerson.name){
        return 1;
      }else if (leftPerson.name < rightPerson.name){
        return -1;
      } else {
        return 0;
      }
...
const hasChildrenEntries = (arr, character) => {
  ...
  for (let i =0; i < arr.length; i++){
    if(arr[i].name === character) {
      let keyValues = Object.entries(arr[i])
      if (keyValues[2][1].length > 0) {
        anyChildren = true;
      }
    }
  }
...
const totalCharacters = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++){
    total.push(arr[i].name);
    if (arr[i].spouse){
      total.push(arr[i].spouse)
    }
    for (let j = 0; j < arr[i].children.length; j++){
      total.push(arr[i].children[j])
    }
  }
  return total.length;
};
...
const houseSize = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++){
    ...
    sizes.push({house: arr[i].house, members: people})
  }
